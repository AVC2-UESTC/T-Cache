...
class MaskGCT_S2A(nn.Module):
    ...
    def reverse_diffusion(
        ...
        assert kwargs is not None,f" Kwargs should be not None, but now is {kwargs}"
        ...
        for mask_layer in range(gt_layer, max_layer):
            ...
            cache_dic, current, cache_dic_cfg, current_cfg = initialize_caching_mode(kwargs, num_steps=steps,prompt_len=prompt_len,phone_len=None,stage_name=stage_name,mask_layer=mask_layer.item())
            ...
            for i in range(steps):
                ...
                embeds = self.diff_estimator(xt_input,
                                              t, 
                                              temp_cond, 
                                              xt_mask,
                                              current=current,
                ...
                if (cfg > 0 and kwargs.cfg_flag):  
                    ...
                    if current['normal_cfg']:  # compute cfg as normal, this is used in baseline mode
                    ...
                    elif current['should_cache']:  # In T-Cache mode, compute the unconditional branch only on cache steps.
                        mask_embeds = self.diff_estimator(
                            cur,
                            t, 
                            temp_cond[:, prompt_len:, :], 
                        ...
                        residual_cache= mask_embeds - embeds  # Cache the residual.
                        pos_emb_std = embeds.std()  # std(g_cond)
                        embeds = embeds + cfg * (embeds - mask_embeds)  # g_cfg
                        rescale_embeds = embeds * pos_emb_std / embeds.std()  # g_final
                        embeds = rescale_cfg * rescale_embeds + (1 - rescale_cfg) * embeds

                    elif current['should_cache']==False:  # this is activated when t-cache is is reuse mode.
                        
                        assert residual_cache is not None, f"residual is {residual_cache}, ensure that the residual has been propery cached"
                        mask_embeds= embeds + residual_cache  # In reuse mode, estimate the unconditional branch from the cached residual.
                    ...
                    else: raise "unknown cfg caching"
